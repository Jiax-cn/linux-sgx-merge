...
{
    return reinterpret_cast<uint8_t*>(sgx_get_wasm_sec_buf_addr() + SGX_WASM_SEC_SIZE - sizeof(uint64_t) - MAX_MAP_ENCL_MR_SIZE);
}

uint8_t *sgx_get_map_encl_mr_addr()
{
    return reinterpret_cast<uint8_t*>(sgx_get_wasm_sec_buf_addr() + SGX_WASM_SEC_SIZE - MAX_MAP_ENCL_MR_SIZE);
}

uint8_t *sgx_get_wasm_blob_addr()
{
    return reinterpret_cast<uint8_t*>(sgx_get_wasm_sec_buf_addr() + (SGX_WASM_SEC_SIZE - *(uint64_t*)sgx_get_wasm_blob_size_addr() - sizeof(uint64_t) - MAX_MAP_ENCL_MR_SIZE) / 4096 * 4096);
}
...
sgx_status_t sgx_wasm_derive_measurement_from_vm_mr_sec(uint8_t *wasm_sec, uint64_t wasm_sec_size, sgx_measurement_t *mr){
	...
	return sgx_wasm_derive_measurement(wasm_sec, wasm_sec_size, vm_mr, mr);
}
...
sgx_status_t sgx_wasm_derive_measurement(uint8_t *wasm_sec, uint64_t wasm_sec_size, sgx_wasm_vm_mr_t *vm_mr, sgx_measurement_t *mr)
{
    ...
    if (wasm_sec_size > SGX_WASM_SEC_SIZE || mr == NULL)
    ...
    memcpy(wasm_sec_buf, wasm_sec, wasm_sec_size);
